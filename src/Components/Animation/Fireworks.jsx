...
const Fireworks = () => {
  ...
  const createParticule = (x, y, ctx) => ({
    ...
    radius: anime.random(getFontSize(), getFontSize() * 2),
    draw() {
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.radius, 0, 2 * Math.PI, true);
  ...
  const createParticles = (x, y, ctx) => {
    const particles = [];
    for (let i = 0; i < numberOfParticules; i++) {
      const particle = createParticule(x, y, ctx);
      particles.push(particle);
    }
    return particles;
  };

  const animateParticules = (x, y, ctx, canvas) => {
    setCanvasSize(canvas);
    const particles = createParticles(x, y, ctx);
    const circle = createCircle(x, y, ctx);

    const particulesAnimation = anime({
      targets: particles,
      x: () => x + anime.random(-distance, distance),
      y: () => y + anime.random(-distance, distance),
      radius: 0,
      duration: () => anime.random(1200, 1800),
      easing: 'easeOutExpo',
      complete: () => removeAnimation(particulesAnimation)
    });
    ...
    const circleAnimation = anime({
      ...
      duration: () => anime.random(1200, 1800),
      easing: 'easeOutExpo',
      complete: () => removeAnimation(circleAnimation)
    });
  ...
  useEffect(() => {
    ...
    const resizeHandler = () => setCanvasSize(canvas);
    const tapHandler = (e) => {
      const x = e.clientX || e.touches?.[0].clientX;
      const y = e.clientY || e.touches?.[0].clientY;
      animateParticules(x, y, ctx, canvas);
    };
